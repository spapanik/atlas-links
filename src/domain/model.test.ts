...
describe('normalization and validation', () => {
    it('normalizes safe URL parts but preserves paths and queries', () => {
        ...
        expect(normalizeUrl('https://EXAMPLE.com/docs/?q=1#x')).toBe(
    ...
    it('normalizes tags case-insensitively and keeps first spelling', () =>
        expect(normalizeTags([' Work ', 'work', '', 'Long   Read'])).toEqual([
            'Work',
            'Long Read',
        ]));
...
describe('merge', () => {
    it('chooses the newer record including a deletion', () => {
        const local = {
            ...emptyStore('2026-01-01T00:00:00.000Z', 'local'),
            bookmarks: [bookmark()],
        };
        ...
        const remote = {
            ...emptyStore('2026-01-02T00:00:00.000Z', 'remote'),
            bookmarks: [deleted],
        };
