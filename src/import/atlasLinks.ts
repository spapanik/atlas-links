...
function parseRecord(value: unknown): AtlasLinksImportRecord {
    ...
    if (record.id !== undefined && (typeof record.id !== 'string' || !record.id.trim()))
        throw new AtlasLinksImportError(
            'The bookmark ID must be a non-empty string when provided.',
        );
    if (
    ...
    if (record.createdAt !== undefined && !isIsoTimestamp(record.createdAt))
        throw new AtlasLinksImportError(
            'The bookmark createdAt value is not a valid UTC timestamp.',
        );
    if (record.updatedAt !== undefined && !isIsoTimestamp(record.updatedAt))
        throw new AtlasLinksImportError(
            'The bookmark updatedAt value is not a valid UTC timestamp.',
        );
...
export function parseAtlasLinksExport(
    ...
    for (const raw of exported.bookmarks) {
        ...
        if (seenIds.has(id))
            throw new AtlasLinksImportError(
                `The export contains the duplicate bookmark ID “${id}”.`,
            );
        seenIds.add(id);
    }
...
export function prepareAtlasLinksImport(
    ...
    > = normalizedRecords.map((record) => {
        ...
        if ((urlCounts.get(record.url) ?? 0) > 1)
            return {
                conflict: {
                    record,
                    reason: 'Multiple imported records use the same normalized URL.',
                },
            };
        ...
        if (idMatch?.deletedAt)
            return {
                conflict: { record, reason: 'The bookmark ID belongs to a deleted local record.' },
            };
    ...
    for (const candidate of candidates) {
        ...
        preview[candidate.kind === 'new' ? 'newBookmarks' : candidate.kind].push(
            candidate.proposal,
        );
    }
    return preview;
