...
export function AtlasLinksImportReview({
    ...
    return (
        <section className="import-review" aria-labelledby="atlas-import-review-title">
            ...
            <p className="muted">
                Missing bookmarks are left unchanged. Conflicted and invalid records will not be
                applied.
            </p>
        </section>
    );
...
export function AtlasLinksJsonImporter({
        ...
        try {
            if (importMode === 'overwrite') {
                ...
                setCompletion({
                    ...
                    notApplied:
                        parsed.invalid.length + preview!.conflicts.length + result.conflicts.length,
                });
            }
        } catch {
            setError(
    ...
    return (
        ...
        >
            ...
            <p>
                Choose an Atlas Links JSON export up to 5 MB and 10,000 records. The file is
            ...
            {importMode === 'overwrite' && parsed && !completion && (
                <section className="import-review" aria-labelledby="atlas-import-review-title">
                    ...
                    <p className="warning">
                        ...
                        {parsed.bookmarks.length} bookmarks from the file. Any bookmarks not in the
            ...
            {completion && (
                <div className="import-complete" role="status" tabIndex={-1} ref={completionRef}>
                    ...
                    <p>
                        {completion.created} added, {completion.updated} updated,{' '}
                        {completion.unchanged} unchanged, and {completion.notApplied} not applied.
                    </p>
                </div>
            )}
                ...
                {!completion && (
                    <button
                        ...
                        disabled={
                            busy ||
                            changeCount === 0 ||
                            (importMode === 'overwrite' && !overwriteChecked)
                        }
                    >
                        {busy
                            ? 'Applying…'
