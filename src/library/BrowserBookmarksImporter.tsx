...
export function BrowserImportReview({ preview }: { preview: BrowserBookmarkPreview }) {
    const details = [
        ...
        ...preview.duplicates.slice(0, 5).map((bookmark) => ({
            label: excerpt(bookmark.name, bookmark.url),
            status:
                bookmark.reason === 'already-saved' ? 'Already saved' : 'Repeated in this export',
        })),
    ...
    return (
        <section className="import-review" aria-labelledby="import-review-title">
            ...
            <p className="muted">
                Imported browser folders are ignored. New bookmarks receive no tags.
            </p>
        </section>
    );
}
...
export function BrowserBookmarksImporter({
    ...
    return (
        ...
        >
            ...
            <p>
                Choose an HTML bookmarks export from Chrome, Edge, Firefox, or Safari. Atlas Links
                reads it only on this device. Files can be up to 5 MB and contain up to 10,000
                links.
            </p>
            <label>
                Bookmarks HTML file
            ...
            {completion && (
                <div className="import-complete" role="status" tabIndex={-1} ref={completionRef}>
                    ...
                    <p>
                        {completion.imported} bookmark{completion.imported === 1 ? '' : 's'}{' '}
                        imported. {completion.skipped} skipped.
                    </p>
                </div>
            )}
