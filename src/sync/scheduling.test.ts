...
describe('scheduleBackgroundSync', () => {
    ...
    it.each([
        ...
        await expect(scheduleBackgroundSync(stateReader(status, dirty), alarms)).resolves.toBe(
            false,
        );
        expect(alarms.create).not.toHaveBeenCalled();
    });
...
describe('persisted sync retry scheduling', () => {
    ...
    it.each(['authorization', 'quota', 'corrupt-remote', 'request'] as const)(
        ...
        async (code) => {
            ...
            await expect(
                scheduleSyncRetry(new SyncError(code), storage, alarms, start),
            ).resolves.toEqual({ scheduled: false, exhausted: false });
