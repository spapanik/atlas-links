...
describe('ChromeIdentityService authorization', () => {
    ...
    it.each([
        [
            'consent denial',
            vi.fn(async () => Promise.reject(new Error('The user did not approve.'))),
        ],
...
describe('GoogleDriveStore', () => {
    ...
    it('uses a cached file ID without listing app data', async () => {
        ...
        await expect(
            new GoogleDriveStore().download('access-token', 'cached-file'),
        ).resolves.toEqual({
            id: 'cached-file',
            store,
        });
    ...
    it('creates the app-data file with the expected multipart request', async () => {
        ...
        const fetchMock = vi
            .fn()
            .mockResolvedValue(
                new Response(JSON.stringify({ id: 'created-file' }), { status: 200 }),
            );
        vi.stubGlobal('fetch', fetchMock);
    ...
    it.each([
        ...
        await expect(new GoogleDriveStore().download('access-token')).rejects.toMatchObject({
            code,
        });
    });
});
    ...
    it('cannot be returned to idle by a token request that finishes after sign out', async () => {
        ...
        expect(
            set.mock.calls.every(
                ([value]) =>
                    (value.syncStatus as { state?: string } | undefined)?.state === 'signed-out',
            ),
        ).toBe(true);
    });
});
...
describe('SyncEngine Drive state and recovery', () => {
    ...
    it.each(['authorization', 'quota'] as const)(
        ...
        async (code) => {
            ...
            await expect(
                new SyncEngine(repository, identity, drive).sync(true),
            ).rejects.toMatchObject({
                code,
            });
