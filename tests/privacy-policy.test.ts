...
describe('privacy policy publication', () => {
    it('identifies Atlas Links and clearly explains its purpose on the homepage', () => {
        ...
        expect(homepage).toMatch(
            /Atlas Links is a local-first Chrome extension for capturing, organising, and\s+searching\s+bookmarks\./,
        );
        expect(homepage).toMatch(/sync them through\s+your\s+own\s+Google Drive/);
    ...
    it('states the sharing limits, user controls, and Google Limited Use commitment', () => {
        expect(sectionText('sharing')).toMatch(
            /shared with Google only.*backup and synchronization/,
        );
        expect(sectionText('sharing')).toMatch(
    ...
    it('is static and provides a safe public contact route', () => {
        ...
        expect(
            policyDocument
                .querySelector('section[aria-labelledby="contact"] a')
                ?.getAttribute('href'),
